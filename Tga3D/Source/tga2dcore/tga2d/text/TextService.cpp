...
float Tga2D::TextService::GetSentenceWidth(Tga2D::Text& aText)
{
...
float Tga2D::TextService::GetSentenceHeight(Tga2D::Text& aText)
{
	if (aText.myText.size() <= 0)
	{
		return 0.0f;
...
bool TextService::Draw(Tga2D::Text& aText, Tga2D::SpriteShader* aCustomShader)
{
	const InternalTextAndFontData* fontData = aText.myFont.myData.get();
	...
	float screenSizeX = static_cast<float>(Engine::GetInstance()->GetTargetSize().x);
	float screenSizeY = static_cast<float>(Engine::GetInstance()->GetTargetSize().y);
	...
	float windowRatio = static_cast<float>(Engine::GetInstance()->GetWindowRatio());
...

	{
		...
		for (int i = 0; i < count; i++)
		{
			...
			spriteInstance.myPivot = { 0.f, 1.f };
