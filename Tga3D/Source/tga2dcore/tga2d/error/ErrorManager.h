...
#ifndef _RETAIL
	#define SETCONSOLECOLOR(color) if (Tga2D::Engine::GetInstance()) Tga2D::Engine::GetInstance()->GetErrorManager().SetConsoleColor(color);
	#define ERROR_PRINT(aFormat, ...) if (Tga2D::Engine::GetInstance()) Tga2D::Engine::GetInstance()->GetErrorManager().ErrorPrint(__FILE__, __LINE__, aFormat, ##__VA_ARGS__);
    #define INFO_PRINT(aFormat, ...) if (Tga2D::Engine::GetInstance()) Tga2D::Engine::GetInstance()->GetErrorManager().InfoPrint(aFormat, ##__VA_ARGS__);
	#define INFO_TIP(aFormat, ...) if (Tga2D::Engine::GetInstance()) Tga2D::Engine::GetInstance()->GetErrorManager().InfoTip(aFormat, ##__VA_ARGS__);
#else
