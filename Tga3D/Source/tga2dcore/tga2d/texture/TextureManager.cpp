...
#include <tga2d/noise/PerlinNoise.h>
...
Texture* TextureManager::GetTexture(const wchar_t* aTexturePath, bool aForceSRGB, bool aForceReload)
{
	...
	if (!loadedTexture)
	{
		Engine::GetInstance()->GetFileWatcher()->WatchFileChange(aTexturePath, std::bind(&Tga2D::TextureManager::OnTextureChanged, this, std::placeholders::_1));
	}
	if (FAILED(hr))
	{
...
void TextureManager::GeneratePerlinNoiseTexture(unsigned short aWidth, unsigned short aHeight)
{
	aWidth;
	aHeight;
#ifdef USE_NOISE
	if (!myNoiseTexture)
	{
		...
		if (FAILED(DX11::Device->CreateTexture2D(&tdesc, &tbsd, myNoiseTexture.ReleaseAndGetAddressOf())))
			return;

		delete[] buf;

		HRESULT hr = DX11::Device->CreateShaderResourceView(myNoiseTexture.Get(), NULL, myNoiseResource.ReleaseAndGetAddressOf());
		if (FAILED(hr))
		{
			return;
		}
	...
	HRESULT result = DX11::Context->Map(myNoiseTexture.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result))
	{
		return;
	}
	...
	DX11::Context->Unmap(myNoiseTexture.Get(), 0);
...
void Tga2D::TextureManager::Init()
{
...
void Tga2D::TextureManager::ReleaseTexture(Texture* aTexture)
{
	aTexture->SetShaderResourceView(myFailedResource.Get());
}

void Tga2D::TextureManager::Update()
{
}
