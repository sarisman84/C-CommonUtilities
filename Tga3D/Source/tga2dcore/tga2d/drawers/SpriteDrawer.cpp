...
void SpriteBatchScope::Draw(const Sprite2DInstanceData* aInstances, size_t aInstanceCount)
{
	...
	for (int i = 0; i < aInstanceCount; i++)
	{
		...
		Matrix2x2f ratioMatrix = Matrix2x2f::CreateScaleMatrix(Vector2f(instance.myScaleSpritesWithAspectRatio ? aspectRatio : 1, 1));
		
		Matrix2x2f m = scalingMatrix * rotationMatrix * ratioMatrix;
		...
		shaderInstance.myAspectRatio = instance.myScaleSpritesWithAspectRatio ? aspectRatio : 1.f;
...
void SpriteBatchScope::Draw(const Sprite3DInstanceData* aInstances, size_t aInstanceCount)
{
	...
	for (int i = 0; i < aInstanceCount; i++)
	{
		...
		shaderInstance.myColor = instance.myColor.AsLinearVec4();
		shaderInstance.myAspectRatio = 1.f;
