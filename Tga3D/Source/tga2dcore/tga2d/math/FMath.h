...
namespace FMath
{
	...
	template <class T>
	::Tga2D::Vector3<T> NLerp(const ::Tga2D::Vector3<T>& a, const ::Tga2D::Vector3<T>& b, float t)
	{
	...
	template <class T>
	::Tga2D::Vector3<T> SLerp(::Tga2D::Vector3<T> start, ::Tga2D::Vector3<T> end, float percent)
	{
		...
		::Tga2D::Vector3<T> RelativeVec = end - start * dot;
		RelativeVec.Normalize();
		// Orthonormal basis
		// The final result.
		::Tga2D::Vector3<T> returnValue= ((start * cos(theta)) + (RelativeVec * sin(theta)));
		return returnValue;
	}
}