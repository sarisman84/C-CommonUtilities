...
struct SpriteResourceEngine
{
	...
	Tga2D::Sprite2DInstanceData myInstanceData = {};
};
...
{
	{
		...
		Tga2D::Text textEngineFPS(L"Text/BarlowSemiCondensed-Regular.ttf", Tga2D::FontSize_18);
		textEngineFPS.SetText("");
		textEngineFPS.SetPosition({ 0.1f, 0.1f });
		textEngineFPS.SetColor(Tga2D::Color(1.0f, 1.0f, 1.0f, 1.0f));

		Tga2D::Text textLoaderFPS(L"Text/BarlowSemiCondensed-Regular.ttf", Tga2D::FontSize_18);
		textLoaderFPS.SetText("");
		textLoaderFPS.SetPosition({ 0.1f, 0.12f });
		textLoaderFPS.SetColor(Tga2D::Color(1.0f, 1.0f, 1.0f, 1.0f));

		Tga2D::Text textGameFPS(L"Text/BarlowSemiCondensed-Regular.ttf", Tga2D::FontSize_18);
		textGameFPS.SetText("");
		textGameFPS.SetPosition({ 0.1f, 0.14f });
		textGameFPS.SetColor(Tga2D::Color(1.0f, 1.0f, 1.0f, 1.0f));
...
void RunLoadingThread()
{
	...
	{
		myToLoadResources.push_back(L"sprites/tga_logo.dds");
	}
	...
	while (!myShouldExit)
	{
		...
		if (myToLoadResources.size() > 0)
		{
			...
			newEngineResource.mySharedData.myTexture = Tga2D::Engine::GetInstance()->GetTextureManager().GetTexture(myToLoadResources[0].c_str());
			newEngineResource.myInstanceData.mySize = newEngineResource.mySharedData.myTexture->CalculateTextureSize();
