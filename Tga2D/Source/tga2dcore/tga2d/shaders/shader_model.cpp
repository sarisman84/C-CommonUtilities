...
bool Tga2D::CShaderModel::CreateInputLayout(ID3D10Blob* aVS)
{
	...
	HRESULT result = myDirect3dEngine->GetDevice()->CreateInputLayout(&layout[0], numElements, aVS->GetBufferPointer(), aVS->GetBufferSize(), myLayout.ReleaseAndGetAddressOf());
	if (FAILED(result))
	{
...
void CShaderModel::SetShaderParameters(CModelShape* aObject, ID3D11Buffer* aObjectBuffer, ID3D11Buffer* aVertexBuffer)
{
	...
	myDirect3dEngine->GetContext()->VSSetConstantBuffers((int)EConstantBufferSlot::Object, 1, &aObjectBuffer);
}