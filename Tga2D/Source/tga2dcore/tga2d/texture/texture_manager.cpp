...
#include <tga2d/noise/perlin_noise.h>
...
CTexture* CTextureManager::GetTexture(const char* aTexturePath, bool aForceReload)
{
	...
	auto it = std::find_if(myResourceViews.begin(), myResourceViews.end(), [hashedID](const std::unique_ptr<CTexture>& s) { return s->myID == hashedID; });
	...
	{
		CTexture* texture = it->get();
		return texture;
	}

	ComPtr<ID3D11ShaderResourceView> resource;
	HRESULT hr = DirectX::CreateDDSTextureFromFile(myDirect3D->GetDevice(), aTexturePath, nullptr, resource.ReleaseAndGetAddressOf());
	if (FAILED(hr))
	{
	...
	if (FAILED(hr))
	{
		...
		SetDebugObjectName(newTexture->myResource.Get(), aTexturePath);
		...
		if (!loadedTexture)
		{
			myResourceViews.push_back(std::unique_ptr<CTexture>(newTexture));
		}
		return newTexture;
	}
	...
	if (resource)
	{
		...
		size_t bitsPerPixel = DirectX::BitsPerPixel(GetTextureFormat(resource.Get()));
...
std::unique_ptr<CRenderTarget> CTextureManager::CreateRenderTarget(const VECTOR2UI& aSize)
{
	HRESULT hr = S_OK;
	...
	hr = device->CreateShaderResourceView(temporaryTexture, nullptr, resource.ReleaseAndGetAddressOf());
	if (FAILED(hr))
	{
	...
	hr = device->CreateRenderTargetView(temporaryTexture, nullptr, target.ReleaseAndGetAddressOf());
	if (FAILED(hr))
	{
	...
	newTarget->mySize = GetTextureSize(resource.Get(), false);

	return std::unique_ptr<CRenderTarget>(newTarget);
} 
...
VECTOR2F CTextureManager::GetTextureSize( ID3D11ShaderResourceView* aResourceView, bool aNormalize)
{
	ID3D11Resource* resource = nullptr;
	aResourceView->GetResource(&resource);
	if (!resource)
...
void CTextureManager::GeneratePerlinNoiseTexture(unsigned short aWidth, unsigned short aHeight)
{
	...
	if (!myNoiseTexture)
	{
		...
		if (FAILED(myDirect3D->GetDevice()->CreateTexture2D(&tdesc, &tbsd, myNoiseTexture.ReleaseAndGetAddressOf())))
			return;

		delete[] buf;

		HRESULT hr = myDirect3D->GetDevice()->CreateShaderResourceView(myNoiseTexture.Get(), NULL, myNoiseResource.ReleaseAndGetAddressOf());
		if (FAILED(hr))
		{
			return;
		}
	...
	HRESULT result = engine.GetContext()->Map(myNoiseTexture.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedResource);
	if (FAILED(result))
	{
		return;
	}
	...
	engine.GetContext()->Unmap(myNoiseTexture.Get(), 0);
...
void CTextureManager::CreateErrorSquareTexture()
{
	...
	hr = myDirect3D->GetDevice()->CreateShaderResourceView(tex.Get(), NULL, myFailedResource.ReleaseAndGetAddressOf());
	if (FAILED(hr))
	{
		return;
	}
...
ComPtr<ID3D11ShaderResourceView> CTextureManager::CreateWhiteSquareTexture()
{
	HRESULT hr;
	...
	ComPtr<ID3D11ShaderResourceView> resource;
	hr = myDirect3D->GetDevice()->CreateShaderResourceView(tex.Get(), NULL, resource.ReleaseAndGetAddressOf());
	if (FAILED(hr))
	{
		return nullptr;
	}
...
void CTextureManager::CreateDefaultNormalmapTexture()
{
	...
	hr = myDirect3D->GetDevice()->CreateShaderResourceView(tex.Get(), NULL, myDefaultNormalMapResource.ReleaseAndGetAddressOf());
	if (FAILED(hr))
	{
		return;
	}
...
void Tga2D::CTextureManager::Init()
{
	...
	myWhiteSquareTexture = std::make_unique<CTexture>();
	myWhiteSquareTexture->myPath = "WhiteSquare";
	myWhiteSquareTexture->myResource = CreateWhiteSquareTexture();
