...
namespace Tga2D
{
	...
	public:
		...
		struct SAnimEvent
		{
			std::map<aiString, SEvent> myEvents;
		};
		...
		std::map<aiString, std::vector<std::function<void()>>> myEventCallbacks;
		void RegisterCallback(const aiString& aEvent, std::function<void()> aFunctionToCall)
		{
		...
		void BoneTransform(Matrix44*& Transforms)
		{
			...
			AnimationTime = std::min(AnimationTime, (float)(myTransformSize[_curScene] -1));
		...
		EModelStatus Update(float dt)
		{
			...
			if (myTotalAnimTime[_curScene] >= realDuration)
			{
				...
				status = EModelStatus::Animation_End;
			}
