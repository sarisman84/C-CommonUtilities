...
int DetermineAndLoadVerticies(aiMesh* fbxMesh, CLoaderMesh* aLoaderMesh)
{
	...
	aLoaderMesh->myVertices = new char[vertexBufferSize * fbxMesh->mNumVertices];
	...
	memcpy(aLoaderMesh->myVertices, &vertexCollection.myData[0], (size_t)vertexBufferSize * (size_t)fbxMesh->mNumVertices);
...
{
	UNREFERENCED_PARAMETER(aModelPath);
	...
	bool hasBones = false;
	for (int meshIndex = 0; meshIndex < (int)aScene->mNumMeshes; meshIndex++)
	{
...
void Tga2D::CMeshNode::SetMap(const EShaderMap, const char* aPath)
{
