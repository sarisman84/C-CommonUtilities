...
bool CCustomShapeDrawer::Init()
{
    ...
    objectBufferDesc.ByteWidth = sizeof( SCustomShapeConstantBufferData );
    ...
    HRESULT result;
    result = myEngine.GetDevice()->CreateBuffer( &objectBufferDesc, NULL, myObjectBuffer.ReleaseAndGetAddressOf() );
    if( FAILED( result ) )
    {
...
int CCustomShapeDrawer::SetShaderParameters( CRenderObjectCustom* aObject )
{
    ...
    HRESULT result = myDirect3dEngine->GetContext()->Map( myObjectBuffer.Get(), 0, D3D11_MAP_WRITE_DISCARD, 0, &mappedObjectResource );
    if( FAILED( result ) )
    {
    ...
    SCustomShapeConstantBufferData* objectDataPtr;
    objectDataPtr = (SCustomShapeConstantBufferData*)mappedObjectResource.pData;
    ...
    myDirect3dEngine->GetContext()->Unmap( myObjectBuffer.Get(), 0 );
	...
	{
		myEngine.GetContext()->PSSetShaderResources(0, 1, shape->GetTexture()->myResource.GetAddressOf());
	}
	
    myDirect3dEngine->GetContext()->VSSetConstantBuffers((int)EConstantBufferSlot::ObjectCustom, 1, myObjectBuffer.GetAddressOf() );
    return tris;
}
